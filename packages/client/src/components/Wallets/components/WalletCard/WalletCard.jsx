...
export function WalletCard({ walletName, id, onRemoveWallet }) {
  ...
  const visitWallet = () => history.push(`${match.path}/${walletName}`);
  const addNewCoin = () => history.push(`${match.path}/${walletName}/add-coin`);
  ...
  const onConfirm = async () => {
    ...
    toggleRemoveModal();
    try {
      const url = `${apiServer}/api/v1/wallets/${id}`;
      ...
      console.log("onDelete walletName", result);
    } catch (err) {
      console.error(err);
    } finally {
