...
export function FormCreateWorker() {
  ...
  const { loading: loadingMiner, error: minerError } = useFetch(
    `${apiServer}/api/v1/miners/_installed`
  );

  const { loading: loadingWallet, error: walletsError } = useFetch(
  ...
  const saveWorker = async state => {
    ...
    const workerAPI = `${apiServer}/api/v1/workers`;
